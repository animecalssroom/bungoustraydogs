...
import { X, Sword, Shield, Zap, Target, AlertCircle, Skull, Activity } from 'lucide-react'
import type { District } from '@/backend/models/district.model'

interface StrikeModalProps {
  district: District
  onClose: () => void
  targets?: { user_id: string; username: string; rank: number; points: number }[]
}
...
export function StrikeModal({ district, onClose, targets = [] }: StrikeModalProps) {
  const [approach, setApproach] = useState<'Frontal Assault' | 'Outmaneuver' | 'Ability Overload'>('Frontal Assault')
  const [selectedTarget, setSelectedTarget] = useState<string | null>(targets[0]?.user_id || null)
    ...
    try {
      const resp = await fetch('/api/war/strike', {
        ...
        body: JSON.stringify({ 
          ...
          warId: district.id, // Assuming district.id maps to the warzone/war_id context or we pass it
          approach 
        }),
      })
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-[120px] overflow-y-auto pr-2 custom-scrollbar">
                    {targets.map((tg) => (
                      ...
                      >
                        <span className="text-[0.65rem] font-bold">@{tg.username.toUpperCase()}</span>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex-1">
                            <h3 className={`text-[0.7rem] font-black uppercase tracking-widest ${isActive ? 'text-white' : 'text-white/40'}`}>
                              {app.name}
                            </h3>
                            <p className="text-[0.55rem] text-white/30 mt-1 leading-tight">{app.desc}</p>
                          </div>
                        </button>
                      )
                    })}
